...
def drive_test():
    url_drive = "https://docs.google.com/spreadsheets/d/1pAciTjKST3Yn0KMnmZM02-xXWwwdJShn/export?format=xlsx"

    print("Conectando a tu Google Drive...")
    try:
        xls = pd.ExcelFile(url_drive)
    except Exception as e:
        print(
            f"Error: Asegúrate de que los permisos del archivo digan 'Cualquier persona con el enlace puede leer'. Detalle: {e}")
        return
    ...
    # Cargamos la primera hoja para probar
    df = pd.read_excel(xls, sheet_name=xls.sheet_names[0])
    print("\n✅ Extracción exitosa de la primera hoja:")
    print(df.head())
