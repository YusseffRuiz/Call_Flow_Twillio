...
# -----------------------------
# Start an outbound call (for quick testing)
# -----------------------------
...
async def twilio_start_call(payload: dict,
    ...
    try:
        ...
        encoded_name = quote(paciente_nombre)
        answer_url = f"{PUBLIC_BASE_URL}/twilio/answer?nombre={encoded_name}"
...
async def twilio_ws(ws: WebSocket):
    ...
    paciente_nombre = params.get("nombre", "Paciente")

    if DEBUG:
        print(f"[WS] Iniciando stream para: {paciente_nombre}")
    ...
    async def send_mulaw_audio(mulaw_bytes: bytes, turn_data=None):
        ...
        try:
            ...
            mark_msg = {
                ...
                "mark": {"name": "bot_termino_de_hablar"}
            }
            await ws_send(mark_msg)
    ...
    try:
        ...
        while True:
            ...
            if event == "start":
                ...
                # Twilio nos confirma que el audio terminó de sonar en el teléfono
                if msg["mark"]["name"] == "bot_termino_de_hablar":
                    if DEBUG:
                        print("[WS] 🗣️ Twilio terminó de reproducir. Bot callado. Escuchando...")

                    # el bot deja de hablar
                    bot_speaking = False

                    # Vaciamos cualquier eco que se haya colado en el buffer
                    detector.reset()
                continue
